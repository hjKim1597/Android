package com.example.androidbasic.activity
...
import com.example.androidbasic.activity.databinding.ActivityMainBinding
...
class MainActivity : AppCompatActivity() {
    ...
    // 엑티비티 콜백을 받기 위한 멤버 변수
    var activityLauncher =
        registerForActivityResult(ActivityResultContracts.StartActivityForResult()) {
            Log.d(TAG, ": 콜백실행됨 $it")
            if (it.resultCode == Activity.RESULT_OK) {

                //성공시 실행할 코드를 동작 - data를 받느게 가능함
...
//        tvView.setOnClickListener{}
        ...
        binding.upBtn.setOnClickListener {
            var cnt = binding.upNum.text.toString().toInt().plus(1)
            binding.upNum.text = cnt.toString()
        }
...
//            startActivity(intent)
    ...
    override fun onRestoreInstanceState(savedInstanceState: Bundle) {
        ...
        binding.upNum.text = cnt.toString() //회전했을때 값의 유지
    }
